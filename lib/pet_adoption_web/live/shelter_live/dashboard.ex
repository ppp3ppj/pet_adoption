        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              <p class="text-sm text-gray-500">Node: <span class="font-mono"><%= @shelter_info.node_id %></span></p>
              <p class="text-sm text-gray-500">Connected Shelters: <span class="font-bold"><%= length(@shelter_info.connected_nodes) %></span></p>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-2xl p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <form phx-submit="add_pet" onkeydown="return event.key != 'Enter';">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label class="block text-gray-700 font-semibold mb-2">Health Status</label>
            <input
              type="text"
        ...
        <div class="flex gap-4">
          <button
            type="submit"
            class="flex-1 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition shadow-lg hover:shadow-xl">
            ✅ Add Pet
          </button>
          <button
            type="button"
      ...
      <%= if @show_application_modal && @selected_pet do %>
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div class="bg-white rounded-lg shadow-2xl p-8 max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <p class="text-gray-600 mb-6"><%= @selected_pet.breed %> • <%= @selected_pet.age %> years • <%= @selected_pet.gender %></p>

            <h3 class="text-xl font-bold text-gray-800 mb-4">Applications (<%= length(@pet_applications) %>)</h3>
                  ...
                  <div class={"border rounded-lg p-4 #{application_border_color(app.status)}"}>
                    <div class="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p class="text-sm text-gray-600"><%= app.applicant_email %> • <%= app.applicant_phone %></p>
                      </div>
                    ...
                    <p class="text-sm text-gray-600">Experience: <%= if app.has_experience, do: "Yes", else: "No" %> | Other Pets: <%= if app.has_other_pets, do: "Yes", else: "No" %> | Home: <%= app.home_type %></p>
                    <p class="text-xs text-gray-500 mt-2">Submitted: <%= format_datetime(app.submitted_at) %></p>
